...
from app.schemas.user_schemas import LoginRequest, UserBase, UserNameUpdate, UserBioUpdate, UserStatusUpdate, UserCreate, UserListResponse, UserResponse, UserUpdate
...
# Additional endpoints to update name fields on user object

@router.put("/users/{user_id}/name", response_model=UserResponse, name="update_user", tags=["User Management Requires (Admin or Manager Roles)"])
async def update_user_name(user_id: UUID, user_update: UserNameUpdate, request: Request, db: AsyncSession = Depends(get_db), token: str = Depends(oauth2_scheme), current_user: dict = Depends(require_role(["ADMIN", "MANAGER"]))):
    """
...
@router.put("/users/{user_id}/bio", response_model=UserResponse, name="update_user", tags=["User Management Requires (Admin or Manager Roles)"])
async def update_user_bio(user_id: UUID, user_update: UserBioUpdate, request: Request, db: AsyncSession = Depends(get_db), token: str = Depends(oauth2_scheme), current_user: dict = Depends(require_role(["ADMIN", "MANAGER"]))):
    """
...
@router.put("/users/{user_id}/status", response_model=UserResponse, name="update_user", tags=["User Management Requires (Admin or Manager Roles)"])
async def update_user_prof_status(user_id: UUID, user_update: UserStatusUpdate, request: Request, db: AsyncSession = Depends(get_db), email_service: EmailService = Depends(get_email_service), token: str = Depends(oauth2_scheme), current_user: dict = Depends(require_role(["ADMIN", "MANAGER"]))):
    """
    Update user prof status
    """
    user_data = user_update.model_dump(exclude_unset=True)
    updated_user = await UserService.update(db, user_id, user_data)
    if not updated_user:
    ...
    await UserService.send_notification_email_prof_status(updated_user, email_service)
